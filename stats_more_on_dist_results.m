...
function stats_more_on_dist_results()
    ...
    config.PLOT_OUT_BASE = 'C:\Users\abg28\ANON_DICOM_DATA\CURRENT\ANON_TRISTAN\anon-processing\results\stats-';
    
    config.AIR_THRESHOLD_LEVELS = [NaN, NaN, 0];%4.0];   % [cm^3] within {air_distance}   Set to [NaN, NaN, 0.0] for n = 52
    ...
    function do_these_plots(config, air_measure, air_distance, dist_measure)
        ...
        T_PRONE_ABOVE_SUPINE_AIR_THRESH  = table();
        T_SUPINE_ABOVE_SUPINE_AIR_THRESH = table();
        ...
        for sr = 1:height(T_SUPINE)
            if T_SUPINE{sr, air_measure} >= config.AIR_THRESHOLD   %% >= for n = 52 figures
                T_SUPINE_ABOVE_SUPINE_AIR_THRESH = cat(1, T_SUPINE_ABOVE_SUPINE_AIR_THRESH, T_SUPINE(sr,:));
                T_PRONE_ABOVE_SUPINE_AIR_THRESH  = cat(1, T_PRONE_ABOVE_SUPINE_AIR_THRESH, T_PRONE(sr,:));
            end        
        end
        ...
        hf6a2 = figure();
        box_plot_and_ttest(hf6a2, cat(2,  T_SUPINE_ABOVE_SUPINE_AIR_THRESH{:,air_measure}, ...
                                          T_PRONE_ABOVE_SUPINE_AIR_THRESH{:,air_measure}), ...
                                          ['Pts. with SUPINE air > ' num2str(config.AIR_THRESHOLD) 'cm^3'], ...
        ...
        hf6b2 = figure();
        box_plot_and_ttest(hf6b2, cat(2,  T_SUPINE_ABOVE_SUPINE_AIR_THRESH{:,dist_measure}, ...
                                          T_PRONE_ABOVE_SUPINE_AIR_THRESH{:,dist_measure}), ...
                                          [config.DIST_MEASURE_TO_USE ' OF GLAND: Pts. with SUPINE air > ' num2str(config.AIR_THRESHOLD) 'cm^3'], ...
                                          'Distortion measure [mm]');
        ...
        hf7a1 = figure();
        plot_as_clusters(hf7a1, T_PRONE_ABOVE_SUPINE_AIR_THRESH{:,air_measure}, ...
                             T_PRONE_ABOVE_SUPINE_AIR_THRESH{:,dist_measure}, ...                           
                             T_SUPINE_ABOVE_SUPINE_AIR_THRESH{:,air_measure}, ...
                             T_SUPINE_ABOVE_SUPINE_AIR_THRESH{:,dist_measure}, ...  
                             [config.DIST_MEASURE_TO_USE ' OF GLAND: Pts. with SUPINE air > ' num2str(config.AIR_THRESHOLD) 'cm^3'], ...
                             ['Volume of air within ' air_distance ' of prostate [cm^3]'], ...
                             'Distortion measure [mm]', ...
                             'Prone', 'Supine', ...
    ...
    function p_ttest = box_plot_and_ttest(hf, pts, f_title, y_label)
        ...
        disp(['n = ' num2str(length(pts(:,1)))]);

        [~, q1] = iqr(pts(:,1));
        [~, q2] = iqr(pts(:,2));

        disp(['mean #1 = ' num2str(mean(pts(:,1))) '+/-' num2str(std(pts(:,1))) ' ' ...
              'median #1 = ' num2str(median(pts(:,1))) '[' num2str(q1(1)) ',' num2str(q1(2)) ']']);
        disp(['mean #2 = ' num2str(mean(pts(:,2))) '+/-' num2str(std(pts(:,2))) ' '...
              'median #1 = ' num2str(median(pts(:,2))) '[' num2str(q2(1)) ',' num2str(q2(2)) ']']);
